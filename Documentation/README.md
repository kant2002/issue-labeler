...
#### About Microsoft.DotNet.GitHubIssueLabeler.Assets nuget package (retired)
...
The public-dispatcher branch is the code base for the github app that gets installed per github organization. Therefore, we would have one app instance per org (one for Microsoft and one for dotnet). The github app would be able to query top three predictions from apps based off of the main branch which are configured for one or more repository to provide predictions.
